create or replace function public.get_subscription_access_state_for_tenant(target_tenant_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
revoke all on function public.get_subscription_access_state_for_tenant(uuid) from public;
grant execute on function public.get_subscription_access_state_for_tenant(uuid) to authenticated;
